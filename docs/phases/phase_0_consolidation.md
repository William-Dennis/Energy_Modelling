...
Consolidate the two parallel strategy frameworks into one. The `backtest.*` framework
...
- [x] Write test: `tests/backtest/test_submission_strategies.py` — test only existing strategies

### 0b. Fix `_backtest.py` discovery
- [x] Change `module_name = f"submission.{stem}"` to `f"strategies.{stem}"` in `_backtest.py:68`
- [x] Verify `import strategies as _pkg` works (line 58)
- [x] Verify `obj.__module__ == module_name` filter still works after rename

### 0c. Port useful metrics into backtest scoring
- [x] Add `profit_factor` to `compute_backtest_metrics()` in `backtest/scoring.py`
- [x] Add `annualized_pnl_eur` to `compute_backtest_metrics()`
- [x] Add `monthly_pnl(daily_pnl)` standalone function to `backtest/scoring.py`
- [x] Add `rolling_sharpe(daily_pnl, window)` standalone function to `backtest/scoring.py`
- [x] Write tests for all new metrics in `tests/backtest/test_scoring.py`
...
- [x] Delete `tests/futures_market/test_market.py`
- [x] Update `src/energy_modelling/futures_market/__init__.py` — remove `MarketEnvironment`, `DayState`, `Settlement`, `Signal`, `Trade` re-exports if only used by deleted code
- [x] Verify `backtest/data.py` still imports from `futures_market.data` correctly
...
- [x] Update `backtest/__init__.py` if any exports changed
- [x] Clean stale `strategy.analysis` comment in `backtest/scoring.py`
- [x] Clean stale `BacktestRunner` comment in `tests/futures_market/test_types.py`
...
- [x] `import energy_modelling.backtest` works
- [x] `import energy_modelling.futures_market` works (data.py, contract.py, types.py)
...
| `src/energy_modelling/strategy/perfect_foresight.py` | Will be rewritten as BacktestStrategy in Phase 4 |
...
| `tests/futures_market/test_market.py` | Tests MarketEnvironment |
...
| `src/energy_modelling/dashboard/_backtest.py` | `submission` -> `strategies` in import path |
...
| `src/energy_modelling/futures_market/__init__.py` | Remove MarketEnvironment re-export |
| `src/energy_modelling/futures_market/types.py` | Remove BacktestRunner docstring ref |
| `src/energy_modelling/backtest/scoring.py` | Add ported metrics |
| `tests/backtest/test_submission_strategies.py` | Rewrite for existing strategies only |
| `tests/backtest/test_scoring.py` | Add tests for new metrics |
...
python -m pytest tests/backtest/test_scoring.py -v

# Run only submission strategy tests
python -m pytest tests/backtest/test_submission_strategies.py -v
...
- No import errors for `strategies`, `energy_modelling.backtest`, `energy_modelling.futures_market`, `energy_modelling.dashboard`
