...
:func:`~energy_modelling.challenge.runner.run_challenge_backtest`, then feeds
...
from energy_modelling.challenge.market import MarketEquilibrium, run_market_to_convergence
...
class MarketEvaluationResult:
    ...
    market_results: dict[str, ChallengeBacktestResult]
    original_results: dict[str, ChallengeBacktestResult]
...
def run_market_evaluation(
    strategy_factories: dict[str, Callable[[], ChallengeStrategy]],
    ...
    1. Run each strategy through ``run_challenge_backtest`` to obtain
       direction predictions and original PnL.
    2. Feed all directions into ``run_market_to_convergence`` to find
    ...
    equilibrium = run_market_to_convergence(
    ...
    for name, orig in original_results.items():
        ...
        metrics = compute_challenge_metrics(market_pnl, trade_count)

        market_results[name] = ChallengeBacktestResult(
            predictions=orig.predictions,
