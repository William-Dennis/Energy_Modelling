...
Runs the synthetic-market model over all challenge strategies and shows
market-adjusted leaderboards, rank changes, convergence diagnostics,
weight evolution, and cumulative market-adjusted PnL.

Expects that the Challenge tab has already been run (results stored in
``st.session_state``).
"""
...
from energy_modelling.challenge.runner import ChallengeBacktestResult
from energy_modelling.challenge.scoring import market_leaderboard_score
from energy_modelling.dashboard._challenge import (
    STRATEGY_FACTORIES,
...
def _market_lb(market_result: MarketEvaluationResult) -> pd.DataFrame:
    rows = []
...
def _render_market_period(period: str, mr: MarketEvaluationResult | None) -> None:
    if mr is None:
...
def render() -> None:
    ...
    val_results = st.session_state.get("challenge_val_results")
    if val_results is None:
        st.info(
            "Run the **Challenge** tab first so that strategy predictions "
            "are available for the market simulation."
        )
        return

    selected = st.session_state.get("challenge_selected", [])
