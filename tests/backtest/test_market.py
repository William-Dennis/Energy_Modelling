...
class TestMarketBehaviour:
    ...
    def test_opposing_strategies_oscillate_when_both_overshoot(self) -> None:
        """Long(110) vs Short(70) with real=100: linear weighting causes oscillation.

        With the spec's linear weights, at market=90:
          Long:  sign(110-90)=+1, profit=+1*(100-90)=10 > 0 => weight=1, price->110
          Short: sign(70-90)=-1,  profit=-1*(100-90)=-10 <= 0 => weight=0
        At market=110:
          Long:  sign(110-110)=0, profit=0 <= 0 => weight=0
          Short: sign(70-110)=-1, profit=-1*(100-110)=10 > 0 => weight=1, price->70
        This alternation never converges — the market oscillates between 110 and 70.
        """
        dates = pd.DatetimeIndex(["2024-01-01"])
        real = pd.Series([100.0], index=dates)
        initial = pd.Series([90.0], index=dates)
        ...
        # With linear weights, opposing strategies oscillate — does NOT converge
