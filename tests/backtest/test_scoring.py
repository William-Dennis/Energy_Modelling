...
class TestComputeMarketAdjustedMetrics:
    def test_includes_base_metrics(self) -> None:
        ...
        base = compute_backtest_metrics(market_pnl, 3)
        for key in base:
...
class TestComputeChallengeMetricsExtended:
    def test_profit_factor_present(self) -> None:
        metrics = compute_backtest_metrics(pd.Series([100.0, -50.0]), trade_count=2)
    ...
    def test_profit_factor_correct(self) -> None:
        ...
        metrics = compute_backtest_metrics(pnl, trade_count=4)
    ...
    def test_profit_factor_inf_when_no_losses(self) -> None:
        ...
        metrics = compute_backtest_metrics(pnl, trade_count=3)
    ...
    def test_annualized_pnl_scales_to_252(self) -> None:
        ...
        metrics = compute_backtest_metrics(pnl, trade_count=252)
    ...
    def test_annualized_pnl_scales_for_partial_year(self) -> None:
        ...
        metrics = compute_backtest_metrics(pnl, trade_count=126)
        assert metrics["annualized_pnl_eur"] == pytest.approx(252.0)

    def test_all_expected_keys_present(self) -> None:
        metrics = compute_backtest_metrics(pd.Series([10.0, -5.0]), trade_count=2)
        expected_keys = {
            "total_pnl",
            "days_evaluated",
            "trade_count",
